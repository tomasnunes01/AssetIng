...
export default function ChangeComputador({ navigation }) {
  ChangeComputador.propTypes = {
  ...
  useEffect(async () => {
    AsyncStorage.getItem('ID').then((fromAsyncId) => {
      EscritorioService.findAll().then((response) => {
        setEscritorioList(response);
      });
      UserService.listar().then((response) => {
        setUserList(response);
      });
      ComputadorService.listarTipos().then((response) => {
        setTipoList(response);
      });
      ComputadorService.findByID(fromAsyncId).then((response) => {
        setNrSerie(response.nr_serie);
        setCodEscritorio(response.cod_escritorio);
        setCodTipo(response.cod_tipo);
        setUsername(response.cod_utilizador);
  ...
  const validar = () => {
    ...
    if (
      !reVarChar20.test(String(sistemaOperativo)) &&
  ...
  const guardar = () => {
    if (validar()) {
      ...
      ComputadorService.atualizar(data)
        .then((response) => {
          setLoading(false);
    ...
    <ThemeProvider theme={theme}>
      ...
      >
        ...
        <View style={styles.header}>
          ...
          <Text style={styles.text_header}>Alterar Computador</Text>
        </View>
        <Animatable.View
